...
class TestCodeBlocks(unittest.TestCase):
    def test_fenced_code_block(self):
        md = "```\ncode here\n```"
        html = markdown_to_html(md, code_block_renderer=lambda block: f"<code-block>{block}</code-block>")
    ...
    def test_fenced_code_block_with_language(self):
        md = "```python\nprint('hello')\n```"
        html = markdown_to_html(md, code_block_renderer=lambda block: f"<code-block>{block}</code-block>")
        self.assertIn("<code-block>```python\nprint('hello')\n```</code-block>", html)

    def test_code_block_renderer_is_required(self):
        md = "```\ncode here\n```"
...
class TestInlineCode(unittest.TestCase):
    def test_inline_code(self):
        md = "Use `code` here"
        html = markdown_to_html(md, code_block_renderer=lambda block: block)
